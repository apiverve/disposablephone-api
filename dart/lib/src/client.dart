...
class DisposablephoneValidationException implements Exception {
  final List<String> errors;

  DisposablephoneValidationException(this.errors);

  @override
  String toString() => 'DisposablephoneValidationException: ${errors.join("; ")}';
}
...
/// Disposable Phone Number Checker API client.
///
/// For more information, visit: https://apiverve.com/marketplace/disposablephone?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [phone] (required) - The phone number to check in E.164 format (e.g., +12085813945)
class DisposablephoneClient {
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'phone': ValidationRule(type: 'string', required: true),
  };

  DisposablephoneClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/disposablephone',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [DisposablephoneValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw DisposablephoneValidationException(errors);
    }
  }

  /// Execute a request to the Disposable Phone Number Checker API.
  ///
  /// Parameters are validated before sending the request.
  Future<DisposablephoneResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw DisposablephoneException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return DisposablephoneResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw DisposablephoneException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw DisposablephoneException('Resource not found');
      } else {
        throw DisposablephoneException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is DisposablephoneException) rethrow;
      throw DisposablephoneException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Disposable Phone Number Checker API client.
class DisposablephoneException implements Exception {
  final String message;

  DisposablephoneException(this.message);

  @override
  String toString() => 'DisposablephoneException: $message';
}
